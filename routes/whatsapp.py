        ...
        if not keyword_matched:
            if has_active_payment:
                ...
                # PAYMENT TRIGGER 2: End of conversation tree (no more templates)
                    ...
                    # User has reached the end of conversation tree and needs AI assistance
                    session["payment_step"] = "awaiting_number"
                    session["pending_question"] = incoming_message
                    redis_service.save_user_session(sender_number, session)

                    # Send payment prompt explaining they need to pay for AI responses
                    ...
                    # User input doesn't match available keywords and there are still templates available
